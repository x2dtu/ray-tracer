...
impl BoundingBox {
    ...
    pub fn hit(&self, r: &Ray, t_min: f64, t_max: f64) -> bool {
        for i in 0..3 {
            let t0 = (self.minimum[i] - r.origin()[i])
                / r.direction()[i].min((self.maximum[i] - r.origin()[i]) / r.direction()[i]);
            let t1 = (self.minimum[i] - r.origin()[i])
                / r.direction()[i].max((self.maximum[i] - r.origin()[i]) / r.direction()[i]);
    ...
    pub fn surrounding_box(box0: &BoundingBox, box1: &BoundingBox) -> BoundingBox {
        ...
        let big = Point::new(
            box0.minimum.x().max(box1.minimum.x()),
            box0.minimum.y().max(box1.minimum.y()),
            box0.minimum.z().max(box1.minimum.z()),
        );
        BoundingBox {
            minimum: small,
            maximum: big,
