...
use crate::{point::Point, hittable_vec::HittableVec, material::Material, rect::{Rect, RectType}, hittable::{Hittable, HitRecord}, ray::Ray, bounding_box::BoundingBox, utility::{self, INF}, vector3::Vector3};
...
impl<T: Material> Cube<T> {
    pub fn new(box_min: Point, box_max: Point, deg_rotation: f64, mat: Rc<T>) -> Cube<T> {
        ...
        Cube { box_min, box_max, sides, sin_theta, cos_theta, _phantom: Default::default() }
    }
}
...
impl<T: Material> Hittable for Cube<T> {
    fn hit(&self, r: &Ray, t_min: f64, t_max: f64) -> Option<HitRecord> {
        ...
        if let Some(rec) = self.sides.hit(r, t_min, t_max) {
            let point = Point::new(self.cos_theta * rec.point.x() + self.sin_theta * rec.point.z(), rec.point.y(), -self.sin_theta * rec.point.x() + self.cos_theta * rec.point.z());
            let outward_normal = Vector3::new(self.cos_theta * rec.normal.x() + self.sin_theta* rec.normal.z(), rec.normal.y(), -self.sin_theta* rec.normal.x() + self.cos_theta * rec.normal.z());
